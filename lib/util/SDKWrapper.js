...
export class SDKWrapper extends EventEmitter {
  ...
  // Aggregates project data needed for intent writing
  async init() {
    try {
      ...
      const messagesFromIntents = new Map();
      for (const { next_message_ids } of board.messages) {
        for (const { message_id, intent } of next_message_ids) {
          if (intent.value) {
            messagesFromIntents.set(message_id, intent.value);
          }
        }
      }
      return { platform, board, intents, messagesFromIntents };
    } catch (err) {
      this.emit('error', err);
    }
  }
