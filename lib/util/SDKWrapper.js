...
export class SDKWrapper extends EventEmitter {
  ...
  async init() {
    try {
      ...
      // reduce the project's intents to a map of intentId -> rest of the payload
      const intents_ = new Map(
        intents.reduce((acc, { id, ...rest }) => [...acc, [id, rest]], [])
      );
      return { platform, board, intents: intents_ };
    } catch (err) {
      this.emit('error', err);
    }
  }
